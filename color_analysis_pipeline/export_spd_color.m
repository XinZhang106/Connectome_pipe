...
if (if_brain)
    ...
    for i = 1:numel(image_datas)
        ...
        if (if_pixel)
            outputFile = fullfile(out_folder, sprintf('im%d_seg%d_color_pixeldata.csv', image_datas(i).image_id, ...
                image_datas(i).seg_id));
        else
            outputFile = fullfile(out_folder, sprintf('im%d_seg%d_color_data.csv', image_datas(i).image_id, ...
                image_datas(i).seg_id));
        end
