function  axon_seg_assgin_by_table(axon_table)
%upload axon tracing to the attach to the sln_image.Image of high-resolution image
...
columns = {'ref_im', 'centroid_x', 'centroid_y', 'centroid_perimeter', 'ml1_x', 'ml1_y', 'ml2_x', 'ml2_y'...
   'spinning_image_id', 'image_folder', 'seg_id'};
...
for j = 1:height(axon_table)
    ...
    imq.seg_id = axon_table.seg_id(j);
    %test if already inserted
    axontest = fetch(sln_image.AxonInBrainV2 & imq);
    ...
    if (isempty(axontest))
        ...
        seg_id = axon_table.seg_id(j);

        sln_image.AxonInBrainV2.assign_axon_in_brain(axon_table.spinning_image_id(j),...
            axon_table.ref_im(j), axon_table.centroid_x(j), axon_table.centroid_y(j), r, ...
            background, mask, medial_lateral, ap, seg_id);
    else
        fprintf('axonal image %d, segment %d already exist in table sln_image.AxonInBrainV2!\n', ...
            imq.image_id, imq.seg_id);
    end
